...
$table_stat_total  = $opt ? get_field('nonteach_table_stat_total', $page_id) : null;
$table_stat_admin  = $opt ? get_field('nonteach_table_stat_admin', $page_id) : null;
$table_stat_support = $opt ? get_field('nonteach_table_stat_support', $page_id) : null;
...
$table_stat_total  = ($table_stat_total !== '' && $table_stat_total !== null) ? (string) $table_stat_total : '11';
$table_stat_admin  = ($table_stat_admin !== '' && $table_stat_admin !== null) ? (string) $table_stat_admin : '5';
$table_stat_support = ($table_stat_support !== '' && $table_stat_support !== null) ? (string) $table_stat_support : '6';
...
?>
    ...
    <!-- Hero Section -->
    <section class="relative px-4 sm:px-6 lg:px-8 pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden bg-gradient-to-br from-primary via-primary-dark to-[#1a1544]">
        ...
        <div class="max-w-7xl mx-auto w-full relative z-10">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
                <div class="flex-1">
                    <div class="inline-flex items-center gap-2 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-4 sm:mb-6">
                        <span class="relative flex h-2 w-2 sm:h-2.5 sm:w-2.5">
                            ...
                            <span class="relative inline-flex rounded-full h-2 w-2 sm:h-2.5 sm:w-2.5 bg-accent"></span>
                        </span>
                    ...
                    <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-[1.1] tracking-tight mb-4 sm:mb-6">
                        <?php echo esc_html($hero_before); ?> <span class="text-transparent bg-clip-text bg-gradient-to-r from-accent via-accent-dark to-accent-light"><?php echo esc_html($hero_highlight); ?></span>
                    </h1>
                ...
                <div class="w-full lg:w-auto grid grid-cols-1 md:grid-cols-3 lg:grid-cols-1 gap-4 lg:gap-6 bg-white/10 backdrop-blur-lg rounded-[1rem] p-4 sm:p-6 border border-white/20 shadow-xl">
                    <?php foreach ($hero_stats as $s) :
                        ...
                        $s_color = $s_icon === 'briefcase' ? 'text-primary-light' : ($s_icon === 'heart' ? 'text-accent-light' : 'text-accent');
                        $s_bg = $s_icon === 'briefcase' ? 'bg-primary-light/20' : ($s_icon === 'heart' ? 'bg-accent-light/20' : 'bg-accent/20');
                    ?>
        ...
        <div class="max-w-7xl mx-auto w-full">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-8 md:mb-12">
                ...
                <div class="relative group flex-1 sm:flex-initial sm:w-auto max-w-sm md:max-w-none">
                    <i data-lucide="search" class="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 size-4 sm:size-5 text-gray-400 group-focus-within:text-primary transition-colors"></i>
                    <input type="text" id="search-input" placeholder="Search..." class="pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3 text-sm sm:text-base rounded-full border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all w-full sm:w-56 md:w-64">
                </div>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mb-6 md:mb-8">
                <div class="bg-gradient-to-br from-primary/10 to-transparent rounded-xl p-3 sm:p-4 border border-primary/20">
                    <p class="text-xs sm:text-sm text-gray-600 mb-1">Total Staff</p>
                    <p class="text-xl sm:text-2xl font-bold text-primary"><?php echo esc_html($table_stat_total); ?></p>
                </div>
                <div class="bg-gradient-to-br from-accent/10 to-transparent rounded-xl p-3 sm:p-4 border border-accent/20">
                    <p class="text-xs sm:text-sm text-gray-600 mb-1">Administrative</p>
                    <p class="text-xl sm:text-2xl font-bold text-accent"><?php echo esc_html($table_stat_admin); ?></p>
                </div>
                <div class="bg-gradient-to-br from-primary-light/10 to-transparent rounded-xl p-3 sm:p-4 border border-primary-light/20 col-span-2 md:col-span-1">
                    <p class="text-xs sm:text-sm text-gray-600 mb-1">Support Staff</p>
                    <p class="text-xl sm:text-2xl font-bold text-primary-light"><?php echo esc_html($table_stat_support); ?></p>
                </div>
            </div>
            <div class="table-container overflow-x-auto rounded-[1rem] border border-gray-200 shadow-lg">
                <table class="min-w-full divide-y divide-gray-200 staff-table">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($staff_rows as $i => $row) :
                            ...
                            $is_principal = (stripos($desig, 'PRINCIPAL') !== false || stripos($desig, 'Executive Head') !== false);
                            $badge_class = $is_principal ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-700';
                        ?>
    ...
    <section class="px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div class="max-w-4xl mx-auto text-center">
            <div class="bg-gradient-to-r from-primary to-primary-dark rounded-[1.5rem] sm:rounded-[1.5rem] p-6 sm:p-8 md:p-12 relative overflow-hidden">
